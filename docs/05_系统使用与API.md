...
用户在“政策文本分析”页输入标题、正文、发布机构、公开时间和来源链接，可选上传 `.txt`/`.md` 文件或临时填写大模型密钥。

系统输出文档编号、处理时间、大模型是否使用、结构化事件、12 个固定谓词、原文证据、六类因素分数、触发规则、基准概率、更新后概率和概率变化。
