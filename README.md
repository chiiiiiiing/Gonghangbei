...
>
> 版本 `AlphaLens-Demo-v2.0-R1` · DeepSeek V4 Flash · 19 谓词固定 Schema · 持久性锚定 Ridge · 冻结验证
>
> **本报告仅供研究参考，不构成投资建议**
...
4. **单篇新文本条件预测**：当期模型输入只包含用户新提交的一篇文本所形成的来源、事件、实体和19谓词特征；历史文本不会与新文本混合计数，只用于训练冻结的 Ridge 参数和规则。产品只输出行业同比增长预测；旧候选因子字段仅在兼容 API 和底层流水线中保留，不进入预测回归，也不在结果页展示。
...
1. **新文本预测首页**：输入正文或链接，使用真实 DeepSeek Key完成AI全链路，直接得到同比预测值、90%区间、加速度和主要特征贡献。
2. **分析报告**：顶部直接展示行业同比预测、加速度与区间；逐股票AI判断、关系证据、19谓词、门控和冻结规则保留在下方审计区，不再展示原候选因子值与个股因子回测；结果末尾展示既定宏观确认策略回测；冻结回放始终标记回放。
...
| v2 研究验证（模型区，桌面） | `演示截图/11_v2_模型验证页_桌面.png` | 合并页上半部分：官方历史目标、单文本模型与持久性基线 |
...
| v2 宏观确认策略回测（桌面） | `演示截图/13_v2_同比预测策略回测_桌面.jpg` | 纯趋势与 AlphaLens 预测确认策略的成本后净值和指标 |
| v2 宏观确认策略回测（移动） | `演示截图/14_v2_同比预测策略回测_移动.jpg` | 390×844 视口，无横向溢出 |
...
| 验证集 | 2022—2023，共23篇；MAE 2.31pct，RMSE 3.08pct | 只用于选择特征集、正则强度和锚定权重 |
| 持久性基线 | 同一验证文本对应目标的MAE 3.01pct | 持久性锚定Ridge验证MAE低约0.70pct（约23.2%） |
...
```bash
.venv/bin/python -m unittest discover -s tests -v   # 54 个测试
...
目标为国家统计局“电气机械和器材制造业增加值同比增速”。模型使用一篇新文本形成的来源、事件、实体、谓词和规则特征，输出预测同比值、相对最新已公布值的加速度、90%区间与主要贡献。1—2月官方合并值作为一个观测，不人工拆分。

- 2015—2021：训练与规则发现；
- 2022—2023：特征集、正则强度、持久性锚权重和策略参数选择验证；
- 2024—最新：冻结样本外观察，不能反向调参；
- 每条训练特征都经过 `published_at <= target_release_date` 检查；预测时只使用当时已经公开的文本和官方值；
- 用户本次输入只形成当期单文本特征，历史文本只用于冻结参数，不与本次文本混合计数；
...
策略是“新能源 ETF `516160` 时间序列动量 + 60日波动率缩放 + AlphaLens 宏观确认”，剩余资金配置 5年期国债 ETF `511010`：

- 月末收盘后计算，下一交易日调仓；
- 12个月新能源收益不为正时，风险仓位为 0；
- 趋势为正时，以2022—2023验证期在预先限定的小参数网格中选择宏观确认规则：预测加速度不低于 `-1.0` 个百分点时使用完整风险缩放仓位，否则风险仓位为 0；该阈值与风险乘数在2024年OOS开始前冻结；
- 目标年化波动率 10%，新能源仓位上限 100%，不做空、不加杠杆；
- 主结果扣除单边 10bp 成本，并报告 5bp、20bp 敏感性；
- 与买入持有、纯趋势、最新已公布值确认、AlphaLens 预测确认比较；Oracle 使用下一期真实值，只作“不可交易”上限，不参与调参或收益宣传。

核心验收是 AlphaLens 增强策略相对纯趋势策略的成本后差异。预测准确但策略无改善时，只能说明具有产业研究价值；预测不准但策略改善时，按偶然或过拟合处理。

当前53个月、单边10bp成本的完整回测中，AlphaLens增强策略年化收益为 **7.26%**、Sharpe为 **0.863**，纯趋势策略分别为 **6.14%** 和 **0.703**。3个月时间块Bootstrap估计的年化净收益差为 **+1.03个百分点**，95%区间约为 **-0.00% 至 3.10%**；因此只能表述为“观察到正增量，统计显著性尚未建立”，不能宣称稳定交易增量。策略参数只使用2022—2023验证期选择，2024年后的OOS不参与调参。
