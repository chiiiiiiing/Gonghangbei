...
2. **分析报告**：展开逐股票AI判断、19谓词、门控、冻结规则和候选因子审计，并下载包含同比预测结论的Markdown报告；冻结回放始终标记回放。
3. **模型验证与审计**：查看106篇历史文本的来源、训练/验证数量、单文本模型与持久性基线误差，以及完整研究审计。
...
```mermaid
flowchart TD
    UI["前端界面<br/>新文本预测首页 / 模型验证 / 研究审计"] --> API["Flask API (app/server.py)"]
...
| v2 模型验证页（桌面） | `演示截图/11_v2_模型验证页_桌面.png` | 官方历史目标、单文本模型与持久性基线验证 |
