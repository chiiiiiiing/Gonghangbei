...
>
> 版本 `AlphaLens-Demo-v1.0-R4` · DeepSeek V4 Flash · 19 谓词固定 Schema · 三层门控 · 34/34 自动化测试通过
>
> **本报告仅供研究参考，不构成投资建议**
...
- **随包视频**：`演示视频/AlphaLens_演示录屏.webm`（55 秒演示：新文本分析 → 储能政策 → 冻结回放 → 谓词融合与候选因子 → 历史研究 → 研究审计）
...
5. **RAG 语义检索**：实时分析检索 163 篇历史 AI 标注结论注入 prompt，参考相似历史研究。
6. **完整审计**：来源、独立文档支持、未来函数检查、`adj_factor=1` 限制、AI 历史缓存覆盖全部如实披露。
...
- 自动化测试：**34/34 通过**（R4 起覆盖抓取、来源评估、置信度校准、冻结回放零破坏），约 5 s 跑完。
- `compileall` 全量编译通过。
...
```bash
.venv/bin/python -m unittest discover -s tests -v   # 34 个测试
