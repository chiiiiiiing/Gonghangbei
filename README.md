# AlphaLens 月度聚合 Nowcast 与产业文本研究平台

> **一句话定位**：AlphaLens 将同月政策、公告、财经新闻和互动问答去重聚合为产业证据，预测国家统计局“电气机械和器材制造业增加值同比增速”；用户输入一篇新文本后，实时 AI 完成结构化审计，并展示该文本加入前后的月度 Nowcast 与配置仓位边际变化。系统预测行业实体景气，**不预测股票价格，不提供投资建议**。
>
> 版本 `AlphaLens-Demo-v2.0-R1` · DeepSeek V4 Flash · 19 谓词固定 Schema · 月度文本残差 Ridge · 冻结验证
>
> **本报告仅供研究参考，不构成投资建议**
...
7. **成熟趋势策略的宏观确认**：新能源 ETF `516160` 使用12个月时间序列动量与60日波动率缩放，AlphaLens只在正趋势下确认完整或半仓；剩余仓位配置5年期国债 ETF `511010`。固定比较买入持有、纯趋势、趋势+已公布宏观、趋势+AlphaLens和不可交易Oracle五组结果。
...
2. **分析报告**：顶部展示本篇文本对本月 Nowcast 的边际影响；逐股票AI判断、关系证据、19谓词、门控和冻结规则保留在下方审计区；结果末尾展示五组趋势与宏观确认策略回测；冻结回放始终标记回放。
...
| v2 趋势策略宏观确认（桌面） | `演示截图/13_v2_同比预测策略回测_桌面.jpg` | 五组固定对照的成本后净值与指标，Oracle醒目标记不可交易 |
...
宏观 Nowcast 的扩展谓词、历史学习规则、AI 动态规则、统一评价协议和冻结边界详见 [宏观规则路线选择规范.md](宏观规则路线选择规范.md)。该规范是规则选择实验的协作合同；验证期未满足门槛时必须显示“文本预测增量不足”。
...
策略是新能源 ETF `516160` 的时间序列动量 + 波动率缩放 + AlphaLens宏观确认；剩余仓位配置5年期国债 ETF `511010`：

- 月末收盘后计算，下一交易日调仓；
- 12个月趋势不为正时新能源仓位为0；趋势为正时再做宏观确认；
- 60日年化波动率缩放至10%目标波动率；宏观加速度为正使用完整风险仓位，否则减半；
- 固定比较买入持有、纯趋势、趋势+最新已公布行业数据、趋势+AlphaLens和趋势+下一期真实值Oracle；
- 新能源仓位限制在0—100%，不做空、不加杠杆；
- 主结果扣除单边 10bp 成本，并报告 5bp、20bp 敏感性；
- 策略参数预先固定，2024年起冻结；Oracle不可交易，只作理论参照。

核心验收是 AlphaLens 增强策略相对纯趋势策略的成本后差异。

当前2024年起冻结OOS、单边10bp成本的27个月回测中：纯趋势年化收益 **8.59%**，AlphaLens增强 **3.08%**。AlphaLens增强相对纯趋势的Bootstrap年化净收益差为 **-5.56个百分点**，95%区间约 **-14.81% 至 0.03%**，覆盖零。当前没有建立交易增量；系统如实保留负结果，不通过收益调参寻找“显著”策略。
