...
文本样本按独立政策事件键计数。2015—2017 年早于市场样本起点的 201 篇文本保留来源与抽取审计，但不回放到 2018 年首个交易日。当前债券供给文本因子在模型期内没有达到5个独立事件，已标记为“证据不足”；该因子和依赖它的规则不会进入模型。五日模型采用5日embargo，并以5个交易日为评估步长，避免正式指标重复计算重叠标签。

每条数据保留来源网址、下载时间和 SHA-256。四条路线共完成380个非重叠滚动预测观测；2025年至最新的81个观测是规则建立后回看的时间留出，不是真正前瞻OOS。规则增强路线相对市场基线的时间留出准确率差为 `-13.58` 个百分点，20个交易日移动区块Bootstrap 95%区间为 `[-22.22%, -7.41%]`，因此当前结论仍为“回顾性时间留出文本预测增量尚未建立”。规则于2026-09-02冻结，真正前瞻OOS从其后的新数据开始累计。
