...
如需演示大模型抽取，公开页面应临时填写当次请求密钥。仅在受信任的内网部署中，才可显式允许服务端密钥：

```bash
export DEEPSEEK_API_KEY="你的密钥"
export ALPHALENS_AI_MODE="api"
export ALPHALENS_ALLOW_SERVER_LLM="true"
```
...
第一条命令下载中债年度国债收益率曲线、中国货币网FDR007历史定盘数据和当日DR007公开行情。第二、三条命令下载2015年至今央行文本，并合并已核验国家统计局历史正文和财政部官方样本。第四条命令生成CPI、PPI、PMI、社融、MLF操作金额和政府债计划发行数据；所有数值按 `release_time` 对齐，事后实际发行量不进入历史特征，政府债计划只在公告后约5个交易日事件窗口内计入。

## 当前真实数据快照

| 数据 | 范围/数量 | 来源 | 用途 |
| --- | --- | --- | --- |
| 10 年期国债收益率 | 2018-01-02 至 2026-09-04，共 2164 个交易日 | 中债收益率曲线 | 主目标与市场特征 |
| FDR007 定盘利率 | 同期 2164 个交易日 | 中国货币网 | DR007 历史公开代理 |
| 政策文本 | 2015-01-22 至 2026-09-01，共 780 篇去重文本 | 人民银行、国家统计局、财政部 | 独立事件、谓词、六类文本因子 |
| 结构化宏观数据 | 2006-01 至最新，共 4669 条观测 | 公开数据接口、人民银行官方公告/PDF与来源哈希审计 | MLF、政府债计划发行可入模；缺少历史 vintage 的镜像/追溯值仅审计 |
| LLM 结构化缓存 | 当前语料匹配404条，其中363条通过门控 | DeepSeek + 原文证据门控 | 可恢复语义抽取与审计 |

文本样本按独立政策事件键计数。2015—2017 年早于市场样本起点的 201 篇文本保留来源与抽取审计，但不回放到 2018 年首个交易日。补入财政部原文后，债券供给因子在模型期内有36个独立事件，已达到5个事件的可入模门槛。五日模型采用5日embargo，并以5个交易日为评估步长，避免正式指标重复计算重叠标签。

每条数据保留来源网址、下载时间和 SHA-256。四条路线共完成381个非重叠滚动预测观测；2025年至最新的82个观测是参数冻结前回看的时间留出，不是真正前瞻OOS。新版先以90%市场基线锚定、叠加10%文本模型，再用0.15固定权重施加规则先验。该时间留出中，文本融合把准确率从57.32%提高到58.54%、Macro-F1从0.3938提高到0.3998；规则增强后进一步达到59.76%和0.4275。最终路线相对市场基线的准确率点估计为 `+2.44` 个百分点，但20个交易日移动区块Bootstrap 95%区间为 `[0.00%, 6.10%]`，仍跨越“无改善”边界。因此准确说法是“观察到正向点估计，但统计证据尚不足”。增强参数于2026-09-07冻结，真正前瞻OOS从冻结后的新数据开始累计。
...
完整说明见 `完整说明文档.md`，现场操作见 `现场演示操作文档.md`，本次增强修正及指标见 `docs/12_文本与规则增强修正说明.md`。
