...
```text
新文本
→ 本地字符 n-gram Embedding 检索相似冻结规则
...
DeepSeek 官方当前没有公开本项目可用的 Embeddings 端点，因此默认在本地把新文本和冻结规则转为 512 维字符 n-gram 向量，以余弦相似度检索前 5 条规则，模型名显示为 `local-char-ngram-embedding-v1`。它是轻量、可复现的词法 Embedding，不应宣称具备专用中文语义模型的同等效果；以后可替换为 BGE 等本地语义向量模型，但不得改变后续接口。
...
- `模式一：大模型候选 + 规则校验`：必须依次完成 Embedding 检索、DeepSeek 结构化 Chat 和程序校验。缺 Key、超时、非法 JSON 或校验失败都会报错，不回退、不生成候选因子。
- `模式二：仅规则复现`：完全不调用模型，用于断网调试和确定性结果复现，页面明确标记为模式二。

## 七、页面怎样讲

1. 指出右侧“AI 研究层待凭证”，说明比赛 Key 由使用者在左侧单次填写，系统不保存。
2. 填写 Key，选择真实政策案例并开始分析。
3. 展示 `local-char-ngram-embedding-v1`、5 条相似冻结规则、模型返回的结构化事件、JSON 输出和通过校验的谓词数量。
...
- 模式一没有配置模型、超时或返回非法 JSON 时直接报错，禁止用确定性结果冒充 AI 结果。
...
Python 和 Playwright 测试会启动本地模拟兼容服务，验证本地 Embedding、Bearer 鉴权、`deepseek-v4-flash`、JSON Object、关闭思考模式、Key 不回显、输入框自动清空、无效 Key 严格报错、非法股票拦截和候选规则状态，不产生真实 API 费用。
