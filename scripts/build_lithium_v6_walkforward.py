"""Build the V6 stricter quality-gated text-alpha report."""
...
from src.lithium.walkforward_v6 import build_report_v6  # noqa: E402


RESEARCH_DIR = ROOT / "data" / "research"
REPORT_FILE = RESEARCH_DIR / "lithium_v6_walkforward_report.json"
ROWS_FILE = RESEARCH_DIR / "lithium_v6_walkforward_rows.csv"
ROW_FIELDS = [
...
def main() -> None:
    ...
    report, rows = build_report_v6(continuous, contracts, signals, texts)
    ...
    with ROWS_FILE.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writeheader()
        writer.writerows(
            {field: row.get(field, "") for field in ROW_FIELDS} for row in rows
        )
