...
def write_progress_doc() -> None:
    ...
    lines = [
        ...
        "- 可演示成果：仓库根目录即演示入口，完整说明见 `完整说明文档.md`",
        ...
        "- Flask Demo：统一使用 `.venv/bin/python 启动演示.py` 启动",
    ...
    lines.extend(
        [
            ...
            "| 第二阶段 | 大模型 API 与结构化输出 | 已自动完成 | `src/ai/` / `参考文档/大模型结构化研究提示词.md` / `完整说明文档.md` |",
            ...
            "| 第六阶段 | Demo 演示与说明 | 已自动完成 | `完整说明文档.md` |",
...
def write_manual_todo_doc() -> None:
    lines = [
        ...
        "- 大模型配置和演示按 `完整说明文档.md` 执行；Key 只填页面密码框，不提交 `.env`，不写进联调记录。",
