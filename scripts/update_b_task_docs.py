...
def write_progress_doc() -> None:
    ...
    lines = [
        ...
        "- AI 研究层：已接入 OpenAI 兼容 Chat/Embedding API、严格 JSON Schema、候选谓词/规则校验和离线规则回退",
        "",
        "## 数据来源分布",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "| 第二阶段 | 大模型 API 与结构化输出 | 已自动完成 | `src/ai/` / `参考文档/大模型结构化研究提示词.md` / `查看材料/大模型API接入与演示手册.md` |",
...
def write_manual_todo_doc() -> None:
    lines = [
        ...
        "| P0 | 配置比赛使用的大模型 API 或本地模型 | 密钥、费用、模型许可和外部文本发送权限必须由团队确认 | 本地 `.env` 和模型连通记录，不提交密钥 |",
        ...
        "- 大模型配置和演示按 `查看材料/大模型API接入与演示手册.md` 执行，禁止提交 `.env`。",
