...
def prepare_demo_script() -> None:
    ...
    lines = [
        ...
        "| 点击开始分析 | 运行结构化抽取和规则校验 | 模型输出必须通过股票池、Schema 和原文证据校验；当前 DeepSeek 路径不启用 Embedding |",
        ...
        "```",
        "",
        "投屏前先准备好 Key。不要把 Key 写进命令、`.env`、PPT 或演示录屏；若需要现场填写，密码框会用掩码显示。没有 Key 或网络异常时，页面自动回退到确定性规则流程。",
        "",
        "## 收尾 20 秒",
        "",
