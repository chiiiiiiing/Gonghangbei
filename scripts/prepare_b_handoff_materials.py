...
def prepare_view_material_index() -> None:
    files = [
        ...
        ("大模型API接入与演示手册.md", "云端或本地模型配置、结构化输出、Embedding 检索和现场演示说明"),
        ...
        ("Demo大模型研究层截图.png", "模拟 OpenAI 兼容服务已连接时的结构化抽取与候选规则截图"),
...
def prepare_c_runbook() -> None:
    ...
    lines.extend(
        [
            ...
            "- 配置模型时检查 `/api/ai/status`，确认 Prompt 版本、对话模型和 Embedding 模型；不得记录或回传 API 密钥。",
...
def prepare_demo_script() -> None:
    ...
    lines = [
        ...
        "| 检查 AI 研究层状态 | 展示模型、Embedding 和 Prompt 版本 | 大模型提出候选，程序负责约束与统计验证 |",
        "| 点击开始分析 | 运行 Embedding、结构化抽取和规则校验 | 模型输出必须通过股票池、Schema 和原文证据校验 |",
