"""Append the latest frozen V5 baseline and text-alpha positions."""
...
LEDGER_FILE = RESEARCH_DIR / "lithium_v5_prospective_decisions.csv"
SOURCE_FILE = ROOT / "src" / "lithium" / "walkforward.py"
...
def append_decision(path: Path, decision: dict[str, Any]) -> str:
    ...
    if path.exists():
        with path.open(encoding="utf-8", newline="") as handle:
            ...
            if reader.fieldnames != FIELDS:
                raise ValueError("V5 前瞻账本字段发生变化，拒绝写入")
    ...
    if matched:
        ...
        if changed:
            raise ValueError("已冻结 V5 决策发生变化: " + ", ".join(changed))
        return "already_recorded"
    if any(item["signal_date"] == row["signal_date"] for item in existing):
        raise ValueError(f"signal_date={row['signal_date']} 已存在其他 V5 决策")
...
def build_decision() -> dict[str, Any]:
    ...
    if latest_index not in baseline:
        raise ValueError("最新交易日缺少 V5 成熟基准仓位")
    ...
    events, _ = quality_text_events(days, signals, texts)
    text_score = active_quality_text_score(latest_index, events)
    baseline_position = float(baseline[latest_index]["position"])
    enhanced_position = (
        max(-1.0, min(1.0, baseline_position + TEXT_WEIGHT * text_score))
    ...
    identity = {
        "strategy_version": "lithium-v5-quality-text-alpha-v1",
